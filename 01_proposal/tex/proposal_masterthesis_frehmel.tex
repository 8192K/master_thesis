...
\englishabstract{Live streaming of 3D Gaussian Splatting (3DGS) scenes captured by fixed multi-camera setups has seen comparatively little research, usually applying restrictive constraints on geo\-me\-try. The proposed thesis aims to study a novel approach lifting these restrictions by adding a velocity vector to 3D Gaussians to predict their movements in combination with updating the 3D scene via differences between two subsequent input images of the same camera only. The proposed thesis focuses on algorithmic feasibility, leaving real-time constraints for future work.}
