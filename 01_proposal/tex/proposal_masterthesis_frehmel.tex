...
\englishabstract{3D Gaussian Splatting (3DGS) comes in two major varieties: either static pre-trained scenes (regular 3DGS) or moving, but still pre-trained scenes (dynamic scene reconstruction/\enquote{4DGS}). A third variety, real-time updates to existing static scenes from e.g. live-feed cameras, has not yet been researched to a deeper extent, mostly due to large hardware requirements and algorithms reaching their limits. We propose a novel approach that intends to allow updating 3DGS scenes in real-time by only using the \textit{differences} between two subsequent input images of the same source camera, creating incremental difference-only 3D scenes from this input and combining them with the existing 3D scene. For this method to be meaningful, we require the scene to be captured in a fixed multi-camera setup.}
...
3DGS is a young technology in the field of 3D Reconstruction which is itself part of the wider Computer Graphics field. It was first published by Kerbl et al. at INRIA in France in 2023 \cite{kerbl3DGaussianSplatting2023} and has since experienced wide adoption and critical acclaim across domains, having been cited more than 10,000 times \cite{semanticScholarKerbl,researchGateKerbl}. 3DGS solved issues previous technologies in 3D Reconstruction had, such as Neural Radiance Fields (NeRFs), as for example finally being able to render 1080p resolution scenes in real-time, making full use of modern GPGPUs, offering editability of the scene and all that at very reasonable training times. 3DGS uses 3D Gaussians as the base primitive, a previously existing data structure which can be described as ellipsoid fuzzy blobs that use a multi-dimensional normal distribution for their opacity, being most opaque at the center and rapidly losing opacity towards the edges. Kerbl et al., among other achievements, made 3D Gaussians differentiable so they can be used in standard backpropagation with gradient descent. This makes 3DGS an explicit radiance field representation without neural networks. My seminar paper covers the base concept and algorithm in much more detail.

\subsection{3D Gaussian Splatting Algorithm}
Regular 3DGS takes 2D input images and learns the pictured 3D scene from them. The training phase can run for 5-20 minutes, depending on scene size but already good results are available within seconds after the start of training. A large outdoor 3D scene can have millions of 3D Gaussians and hundreds of input images. The input images are first converted into a point cloud and camera matrices (camera orientation in space etc.) and the points of the point cloud become the origins for the initial 3D Gaussians before training. It is important to know that even initialising 3D Gaussians randomly still converges, but takes longer to train. 

\subsection{Dynamic Scence Reconstruction Algorithms}
Dynamic scene reconstruction (4DGS) learns a scene for each frame of videos that are being recorded from fixed or moving locations. For a moving camera location, camera matrices have to be calculated for each frame while it has to be done only once for a fixed camera location. Training the initial scene is identical to regular 3DGS, but subsequent frames can rely on the scene geometry from the previous frame, making training a lot faster as 3D Gaussians are already in the vicinity, with similar shape and color of the next frame's content. It can still not be done in real-time, though, as the input imagery is still full 2D images and the full 3D scene is being considered for training. The difference of the 3D scene between frames can be encoded into so-called \enquote{deformation fields} which allow transforming 3D Gaussians over time according to the field. The other option is to encode the time component directly into the 3D Gaussian data structure, effectively transforming them into 4D Gaussians. This means that 4DGS scenes can only be learned frame by frame, encoded and then later replayed and explored. There is no real-time component.
