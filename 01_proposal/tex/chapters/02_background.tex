...
Dynamic scene reconstruction (as in \enquote{reconstructing a dynamic scene}) in general represents reconstruction of 3D scenes that change over time, which means the inputs are video feeds with a certain frame rate. 

\subsubsection{Camera Setups for Dynamic Scene Reconstruction}
\label{ch:camera}
In general, dynamic scene reconstruction scenarios can be divided into single and multi-camera setups using fixed or mobile cameras. Mobile cameras add much to the general computational load as their position has to be estimated with every frame, which is why using fixed camera setups can help bound the load. This automatically limits the extent of the scene but comes naturally for e.g.\,confined indoor spaces.

Single camera setups can be found in 3DGS solutions to SLAM (Simultaneous Localization and Mapping) in robotics which use mobile cameras and live 3D telepresence with fixed cameras for example (e.g. \cite{SplaTAM, TeleGS}). It is clear that with single cameras much of the environment has to be inferred which poses a very different problem.

%For the proposed thesis a fixed multi-camera setup is used in order to be able to compare consecutive frames in a meaningful way.
Another option within this differentiation is so-called \textit{event-based 3DGS} (e.g. \cite{event3DGS}) which uses \enquote{event cameras} (dynamic vision sensors) that only record changes in light intensity, without color. State-of-the-art solutions for SLAM or robotics in general use these kind of cameras allowing real-time 3D reconstruction of unconstrained scenes, however, at lower resolutions and without color for internal representations only.
%he proposed thesis takes a different approach, but solutions in SLAM, especially with event-based 3DGS, could prove to be valuable input, too.

Another option fascilitating dynamic reconstruction are RGB-D cameras, which record depth information alongside color values (e.g. \cite{SplaTAM}). This however works only for small scenes or for short distances as larger scenes require Lidar-based sensors, which have their own restrictions and side-effects.

The proposed thesis makes use of fixed multi-camera setups with regular RGB cameras in order to be able to compare consecutive frames in a meaningful way. As the goal is to achieve online dynamic scene reconstruction, algorithmic details used in event-based SLAM could, however, also prove to be valuable input.

\subsubsection{Offline Dynamic Scene Reconstruction}
In the context of 3DGS as a technology, dynamic scene reconstruction is also called \enquote{4DGS} or \enquote{Free Viewpoint Video} (e.g. \cite{yangDeformable3DGaussians2024, wu4DGaussianSplatting2024,yangRealtimePhotorealisticDynamic2024}). Training the initial scene is identical to regular 3DGS, but the following frames can rely on scene geometry from the previous frame, making training a lot faster as 3D Gaussians are already in the vicinity, with similar shape and color of the next frame's content.
The input imagery is still full 2D images and the full 3D scene is being considered for training.
...
This means that 4DGS scenes are learned from pre-recorded imagery, then encoded and then later replayed and explored with novel-view synthesis.
...
Recent works primarily focus on optimizing compression as a 4DGS scene can become very large quickly \cite{lee2025optimizedminimal4dgaussian} or clustering 3D Gaussians by their motion \cite{NEURIPS2025_b690b88d}. The principle algorithm of using the previous state of the scene as the starting point for the next frame has remained unchanged since inception.

\subsubsection{Online and Real-Time Dynamic Scene Reconstruction}
Contrary to 4DGS scenes, online dynamic scene reconstruction (also called \enquote{3DGS Live Streaming}, \enquote{Live 3DGS}, or \enquote{3DGS Live Video}\footnote{In 3DGS research, it seems common for authors to reinvent terminology.}) describes concurrent reconstruction and novel-view synthesis of incoming video frames, usually after initial regular training of a base 3DGS scene.
...
A problem for both, offline and online dynamic scene reconstruction, is occlusion handling. There will inevitably be parts in the scene that have not been captured by any camera as other geometry completely obstructs the views. For these regions, there are essentially three possibilities:
\begin{itemize}
\item Leave as is. The most honest approach.
\item Use AI priors to fill the region with geometry that makes the most sense.
\item Use statistical solutions to fill the region.
\end{itemize}

\paragraph{Constrained Solutions} Single-camera setups described in Chapter \ref{ch:camera} naturally have to use some kind of prediction mechanism to show what is behind the primary geometry the camera can capture. A prediction is always a constraint as only those things can be predicted which have been trained beforehand. The same applies to skeletal models (usually of humans) or immutable static backgrounds as for example in \cite{huangEchoesColiseum3D2025}. Using such constraints and large processing pipelines with corresponding GPU hardware makes real-time dynamic scene reconstruction possible - but such scenes are severely limited in content, amongst others.

\paragraph{Unconstrained Solutions} Unconstrained online dynamic scene reconstruction has been possible as early as 2023 \cite{luitenDynamic3DGaussians2024} when 3D Gaussians were modelled as physical particles. Later frameworks like 3DGStream \cite{sun3DGStreamOntheFlyTraining2024} and HiCom \cite{gaoHiCoMHierarchicalCoherent2024} advanced this to achieve a much lower error rate as with \cite{luitenDynamic3DGaussians2024} but training one frame would still take seconds to minutes. This also means that real-time dynamic scene reconstruction has so far not been achieved for unconstrained/\allowbreak arbitrary scenes. This thesis proposal tries to take one step towards reaching this goal.
